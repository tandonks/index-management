...
class RollupRunnerIT : RollupRestTestCase() {
    ...
    fun `test rollup on rollup with invalid metric fields`() {
        val sourceIdxName = "source_rollup_invalid"
        val firstRollupIdxName = "first_rollup_invalid"
        val secondRollupIdxName = "second_rollup_invalid"

        generateNYCTaxiData(sourceIdxName)

        // Create first level rollup
        val firstRollup = Rollup(
            id = "first_rollup_invalid_job",
            schemaVersion = 1L,
            enabled = true,
            jobSchedule = IntervalSchedule(Instant.now(), 1, ChronoUnit.MINUTES),
            jobLastUpdatedTime = Instant.now(),
